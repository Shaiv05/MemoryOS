      ...
      <main className="flex-1 p-10 overflow-y-auto">
        <div className="max-w-5xl mx-auto">
          <div className="flex justify-between items-center mb-10">
            ...
            <button
              onClick={() => setEditingGoal({ title: "", description: "", progress: 0, status: "planned", priority: "medium" })}
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="mt-2 flex flex-wrap gap-2 text-[10px] uppercase tracking-widest">
                          <span className="rounded-full border border-zinc-700 bg-zinc-950 px-2.5 py-1 text-zinc-300">{goal.status.replace("_", " ")}</span>
                          <span className="rounded-full border border-zinc-700 bg-zinc-950 px-2.5 py-1 text-zinc-300">Priority: {goal.priority}</span>
                        </div>
                        {(goal.target_date || goal.deadline) && (
                          <p className="text-xs text-zinc-500 mt-2">Deadline: {new Date(goal.target_date || goal.deadline || "").toLocaleDateString()}</p>
                        )}
                      </div>
                    </div>
            ...
            <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-6 z-50">
              ...
              >
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-1 block">Status</label>
                      <select
                        value={editingGoal.status || "planned"}
                        onChange={(e) => setEditingGoal({ ...editingGoal, status: e.target.value as "planned" | "in_progress" | "completed" | "paused" })}
                        className="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-3 focus:outline-none focus:border-zinc-500 transition"
                      >
